...
class IPTVApp {
    ...
    updateCacheStatus(step, total, name) {
        ...
        if (step === 0 || !name) {
            ...
            el.textContent = I18n.t('home.cacheLoading', { step: step, total: total, name: name });
    ...
    autoConnect() {
        ...
        if (playlist.type === 'provider') {
            ...
            this.api.authenticate().then(function() {
                ...
                // Don't hide loading yet - wait for cache to complete
                clearTimeout(loadingTimeout);
                // Preload streams cache with progress indicator
    ...
    playSeriesFromHistory(historyData) {
        ...
        this.currentSeason = historyData._season;
        this.currentEpisodeId = episodeId;
        this.currentEpisodeNum = historyData._episode;
        var stream = {
            stream_id: episodeId,
            series_id: seriesId,
    ...
    showContinueInGrid() {
        ...
        // Log duplicates detection
        var seriesIds = {};
        filteredHistory.forEach(function(item) {
            if (item.seriesId) {
                if (seriesIds[item.seriesId]) {
                    window.log('DUPLICATE in Continue: seriesId=' + item.seriesId + ' name=' + item.name);
                }
    ...
    showTntInGrid() {
        ...
        if (tntStreams.length === 0) {
            container.innerHTML = '<div style="color:#888;font-size:24px;padding:40px;">' + I18n.t('errors.noData') + '</div>';
            return;
        }
    ...
    showTVGuide() {
        ...
        this.guideTimeOffset = 0; // hours offset from current time
        // Get TNT channels first, then fill with other channels
        var allStreams = this.getStreams('live');
        var tntChannels = I18n.getTntChannels();
        var tntStreams = this.getTntStreams(allStreams, tntChannels);
        // Add more channels if needed (up to 20)
        var otherStreams = allStreams.filter(function(s) {
            return tntStreams.indexOf(s) === -1;
        });
        this.guideChannels = tntStreams.concat(otherStreams).slice(0, 20);
        // Render time bar
        this.renderGuideTimeBar();
        // Render channels
        this.renderGuideChannels();
        // Load EPG data for all channels
        this.loadGuideEPG();
    }
    ...
    renderGuideTimeBar() {
        ...
        var now = new Date();
        var startHour = now.getHours() + this.guideTimeOffset;
        var html = '';
        // Show 6 hours
        for (var i = 0; i < 6; i++) {
            ...
            var isCurrent = (i === 0 && this.guideTimeOffset === 0);
    ...
    renderGuideChannels() {
        var container = document.getElementById('guide-channels');
        var html = '';
        for (var i = 0; i < this.guideChannels.length; i++) {
            var ch = this.guideChannels[i];
            var isFocused = (i === this.guideRowIndex);
            html += '<div class="guide-channel' + (isFocused ? ' focused' : '') + '" data-index="' + i + '">';
            html += '<div class="guide-channel-logo" style="background-image:url(\'' + (ch.stream_icon || '') + '\')"></div>';
            html += '<div class="guide-channel-name">' + (ch.name || '') + '</div>';
            html += '</div>';
        }
        container.innerHTML = html;
    }
    ...
    loadGuideEPG() {
        var self = this;
        var container = document.getElementById('guide-programs');
        container.innerHTML = '<div id="guide-programs-scroll"></div>';
        var scrollContainer = document.getElementById('guide-programs-scroll');
        // Create rows for each channel
        for (var i = 0; i < this.guideChannels.length; i++) {
            var row = document.createElement('div');
            row.className = 'guide-row';
            row.dataset.rowIndex = i;
            row.innerHTML = '<div style="padding:20px;color:#666;">Chargement...</div>';
            scrollContainer.appendChild(row);
        }
        // Load EPG for each channel
        var loaded = 0;
        this.guideChannels.forEach(function(ch, idx) {
            self.api.getShortEPG(ch.stream_id, 10).then(function(data) {
                self.guideEpgData[ch.stream_id] = data.epg_listings || [];
                self.renderGuideRow(idx);
                loaded++;
                if (loaded === self.guideChannels.length) {
                    self.updateGuideFocus();
                }
            }).catch(function(err) {
    ...
    renderGuideRow(rowIndex) {
        var ch = this.guideChannels[rowIndex];
        var epg = this.guideEpgData[ch.stream_id] || [];
        var row = document.querySelector('.guide-row[data-row-index="' + rowIndex + '"]');
        if (!row) return;
        var now = Math.floor(Date.now() / 1000);
        var startTime = now + (this.guideTimeOffset * 3600);
        var endTime = startTime + (6 * 3600); // 6 hours window
        var containerWidth = row.parentElement.offsetWidth || 1400;
        ...
        for (var i = 0; i < epg.length; i++) {
            ...
            // Skip programs that end before our window or start after
            if (progEnd < startTime || progStart > endTime) continue;
            // Clamp to visible window
            var visibleStart = Math.max(progStart, startTime);
            var visibleEnd = Math.min(progEnd, endTime);
            // Calculate position
            var left = ((visibleStart - startTime) / timeRange) * containerWidth;
            var width = ((visibleEnd - visibleStart) / timeRange) * containerWidth;
            if (width < 30) continue; // Skip too small programs
            var isLive = (now >= progStart && now < progEnd);
            var title = prog.title || '';
            try {
                title = decodeURIComponent(escape(atob(title)));
            } catch (e) {}
            var startDate = new Date(progStart * 1000);
            var endDate = new Date(progEnd * 1000);
            var timeStr = (startDate.getHours() < 10 ? '0' : '') + startDate.getHours() + ':' +
            ...
            html += '<div class="guide-program' + (isLive ? ' live' : '') + '" ';
            html += 'data-row="' + rowIndex + '" data-prog="' + programIdx + '" ';
            html += 'data-start="' + progStart + '" data-end="' + progEnd + '" ';
            html += 'data-title="' + title.replace(/"/g, '&quot;') + '" ';
            html += 'data-desc="' + (prog.description || '').replace(/"/g, '&quot;') + '" ';
            html += 'style="left:' + left + 'px;width:' + width + 'px;">';
            html += '<div class="guide-program-title">' + title + '</div>';
            html += '<div class="guide-program-time">' + timeStr + '</div>';
            html += '</div>';
            programIdx++;
        }
        // Add now line
        if (this.guideTimeOffset === 0) {
            var nowPos = ((now - startTime) / timeRange) * containerWidth;
            html += '<div class="guide-now-line" style="left:' + nowPos + 'px;"></div>';
        }
        row.innerHTML = html || '<div style="padding:20px;color:#666;">' + I18n.t('errors.noData') + '</div>';
    }

    updateGuideFocus() {
        // Update channel focus
        var channels = document.querySelectorAll('.guide-channel');
        channels.forEach(function(ch, idx) {
            ch.classList.toggle('focused', idx === this.guideRowIndex);
        }, this);
        // Update program focus
        var programs = document.querySelectorAll('.guide-program');
        programs.forEach(function(prog) {
            prog.classList.remove('focused');
        });
        var currentRow = document.querySelector('.guide-row[data-row-index="' + this.guideRowIndex + '"]');
        if (currentRow) {
            var progs = currentRow.querySelectorAll('.guide-program');
            if (progs.length > 0) {
                // Clamp program index
                ...
                var focusedProg = progs[this.guideProgramIndex];
                if (focusedProg) {
                    focusedProg.classList.add('focused');
                    // Update info panel
                    var title = focusedProg.dataset.title || '';
                    var desc = focusedProg.dataset.desc || '';
                    try {
                        desc = decodeURIComponent(escape(atob(desc)));
                    } catch (e) {}
                    var startTs = parseInt(focusedProg.dataset.start, 10);
                    var endTs = parseInt(focusedProg.dataset.end, 10);
                    ...
                    var timeStr = (startDate.getHours() < 10 ? '0' : '') + startDate.getHours() + ':' +
                                  ...
                                  (endDate.getHours() < 10 ? '0' : '') + endDate.getHours() + ':' +
                                  (endDate.getMinutes() < 10 ? '0' : '') + endDate.getMinutes();
                    document.getElementById('guide-info-title').textContent = title;
                    document.getElementById('guide-info-time').textContent = timeStr;
                    document.getElementById('guide-info-desc').textContent = desc;
                }
            }
        }
        // Scroll channel list to keep focused in view
        var channelContainer = document.getElementById('guide-channels');
        var focusedChannel = channels[this.guideRowIndex];
        if (focusedChannel && channelContainer) {
            var channelTop = focusedChannel.offsetTop;
            var channelHeight = focusedChannel.offsetHeight;
            var containerScroll = channelContainer.scrollTop;
            var containerHeight = channelContainer.offsetHeight;
            if (channelTop < containerScroll) {
                channelContainer.scrollTop = channelTop;
            }
            if (channelTop + channelHeight > containerScroll + containerHeight) {
                channelContainer.scrollTop = channelTop + channelHeight - containerHeight;
            }
        }
        // Scroll program list similarly
        var programContainer = document.getElementById('guide-programs-scroll');
        var focusedRow = document.querySelector('.guide-row[data-row-index="' + this.guideRowIndex + '"]');
        if (focusedRow && programContainer) {
            var rowTop = focusedRow.offsetTop;
            var rowHeight = focusedRow.offsetHeight;
            var scrollTop = programContainer.scrollTop;
            var scrollHeight = programContainer.offsetHeight;
            if (rowTop < scrollTop) {
                programContainer.scrollTop = rowTop;
            }
            if (rowTop + rowHeight > scrollTop + scrollHeight) {
                programContainer.scrollTop = rowTop + rowHeight - scrollHeight;
            }
        }
    }
    ...
    navigateGuide(direction) {
        switch (direction) {
            ...
            case 'left':
                if (this.guideProgramIndex > 0) {
                    this.guideProgramIndex--;
                    this.updateGuideFocus();
                } else if (this.guideTimeOffset > -24) {
                    // Scroll time window earlier
                    this.guideTimeOffset -= 3;
                    this.renderGuideTimeBar();
                    for (var i = 0; i < this.guideChannels.length; i++) {
                        this.renderGuideRow(i);
                    }
                    this.guideProgramIndex = 0;
                    this.updateGuideFocus();
                }
            ...
            case 'right':
                var currentRow = document.querySelector('.guide-row[data-row-index="' + this.guideRowIndex + '"]');
                var progs = currentRow ? currentRow.querySelectorAll('.guide-program') : [];
                if (this.guideProgramIndex < progs.length - 1) {
                    this.guideProgramIndex++;
                    this.updateGuideFocus();
                } else if (this.guideTimeOffset < 24) {
                    // Scroll time window later
                    this.guideTimeOffset += 3;
                    this.renderGuideTimeBar();
                    for (var i = 0; i < this.guideChannels.length; i++) {
                        this.renderGuideRow(i);
    ...
    selectGuideProgram() {
        var ch = this.guideChannels[this.guideRowIndex];
        if (!ch) return;
        var focusedProg = document.querySelector('.guide-program.focused');
        var now = Math.floor(Date.now() / 1000);
        if (focusedProg) {
            var progStart = parseInt(focusedProg.dataset.start, 10);
            var progEnd = parseInt(focusedProg.dataset.end, 10);
            // If program is in the past and catchup is available, play catchup
            if (progEnd < now && (ch.tv_archive === 1 || ch.tv_archive === '1')) {
                var duration = Math.ceil((progEnd - progStart) / 60); // in minutes
    ...
    showHistoryScreen() {
        ...
        this.showScreen('browse');
        this.currentScreen = 'browse';
        document.getElementById('sidebar-title').textContent = I18n.t('home.history');
        document.getElementById('filters-bar').style.display = '';
        document.getElementById('search-filters').style.display = 'none';
        document.getElementById('sort-filters').style.display = 'none';
        document.getElementById('sidebar').style.display = 'none';
        ...
        var container = document.getElementById('categories-list');
        container.innerHTML = '';
        // Build history items from watchHistory (deduplicate by item+day)
        var historyItems = [];
        var seen = {};
        for (var i = 0; i < this.watchHistory.length; i++) {
            var item = this.watchHistory[i];
            var dayKey = this.getHistoryDayKey(item.date || 0);
            var itemKey = item.id + '_' + dayKey;
        ...
        if (historyItems.length === 0) {
            document.getElementById('content-grid').innerHTML = '<div style="color:#888;font-size:24px;padding:40px;">' + I18n.t('home.noHistory') + '</div>';
        }
        else {
    ...
    goBack() {
        ...
        if (this.currentScreen === 'player') {
            ...
            this.currentScreen = 'browse';
            this.focusArea = 'sidebar';
            // Find the guide category and focus on it
            var guideIdx = 0;
            var cats = document.querySelectorAll('#categories-list .category-item');
            cats.forEach(function(c, i) {
                if (c.dataset.categoryId === 'guide') guideIdx = i;
            });
