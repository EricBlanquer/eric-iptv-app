...
IPTVApp.prototype.removeHistoryAtIndex = function(index) {
    ...
    var self = this;
    this.showConfirmModal(I18n.t('settings.confirmDeleteHistory', { name: name }), function() {
    ...
    if (this.removeFromHistory(id)) {
        ...
        setTimeout(function() {
            ...
            if (self.currentStreams.length === 0) {
                grid.innerHTML = '<div style="color:#888;font-size:24px;padding:40px;">' + I18n.t('home.noHistory') + '</div>';
            }
        }, 200);
    }
};
